...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Could not read Excel file. Use .xlsx or .xlsm.' }, { status: 400 })
  }
  ...
  if (supabase) {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Saved to file but Supabase update failed. Run the SQL and ensure SUPABASE_SERVICE_ROLE_KEY is set.' },
        { status: 500 }
      )
    }
  }

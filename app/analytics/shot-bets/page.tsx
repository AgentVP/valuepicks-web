...
export default function ShotBetsPage() {
  ...
  async function loadData(accessToken: string) {
    ...
    if (res.status === 401) {
      router.replace('/analytics/login?redirect=/analytics/shot-bets')
      return
    }
    if (!res.ok) {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <p className="mt-6 text-sm text-[var(--ice)]/60">
        Data is stored in <strong>Supabase</strong> after upload (and optionally in <code className="bg-white/10 px-1 rounded">data/shot-bet-analytics.json</code>). The app reads from Supabase first. Use the sheet name that matches your workbook.
      </p>
    </div>
  )
}
